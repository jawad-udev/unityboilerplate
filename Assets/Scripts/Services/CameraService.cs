...
public class CameraService : ICameraService
{
    ...
    {
        _gameService.GetPlayerManager(owner).gameplayCamera = Camera.main;
        Vector3 camPosition = _gameService.GetPlayerManager(owner).transform.position;
    ...
    {
        _gameService.GetPlayerManager(gameplayOwner).ShakeCamera();
    }
    ...
    {
        ScreenUtility screenUtility = _gameService.GetPlayerManager(gameplayOwner).gameplayCamera.GetComponent<ScreenUtility>();
        return new Vector3(0, screenUtility.Top, 0);
    }
    ...
    {
        if (_gameService.CurrentGameManager != null)
            _gameService.CurrentGameManager.gameplayCamera.orthographicSize = _zoomOutLimit;
    }
}